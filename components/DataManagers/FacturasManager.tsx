...
export const FacturasManager: React.FC<FacturasManagerProps> = ({ facturas, datosFiscales, onAddFactura, onUpdateFactura, onDeleteFactura, onGoToDashboard }) => {
  ...
  const handlePrintList = useCallback(() => {
    ...
    try {
        ...
        printDataTable('Informe de Facturas', ["Nº", "Fecha", "Tipo", "Cliente", "Total (€)", "Pagada"], sorted, (f: Factura) => [f.numeroFactura, new Date(f.fecha).toLocaleDateString('es-ES'), f.tipo === 'emitida' ? 'Emitida' : 'Recibida', f.clienteProveedor, f.totalFactura.toFixed(2), f.pagada ? 'Sí' : 'No']);
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingFactura ? 'Editar Factura' : 'Crear Factura'} size="xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label="Número de Factura" name="numeroFactura" value={formData.numeroFactura} onChange={handleChange} required />
          ...
          <Input label="Nombre del Cliente/Proveedor" name="clienteProveedor" value={formData.clienteProveedor} onChange={handleChange} required />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label="NIF/CIF Cliente/Proveedor" name="clienteProveedorNif" value={formData.clienteProveedorNif || ''} onChange={handleChange} />
            <Input label="Dirección Cliente/Proveedor" name="clienteProveedorDireccion" value={formData.clienteProveedorDireccion || ''} onChange={handleChange} />
          </div>
          <TextArea label="Descripción" name="descripcion" value={formData.descripcion} onChange={handleChange} required rows={3}/>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <Input label="Base Imponible (€)" name="baseImponible" type="number" step="0.01" value={formData.baseImponible} onChange={handleChange} required />
            <Input label="IVA (%)" name="ivaPorcentaje" type="number" step="0.01" value={formData.ivaPorcentaje} onChange={handleChange} required />
